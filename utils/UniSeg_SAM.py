...
def adaptive_dempster_fusion(confidence_map_fg, confidence_map_bg, eps=1e-6):
    p_fg = confidence_map_fg.clamp(eps, 1 - eps)
    p_bg = confidence_map_bg.clamp(eps, 1 - eps)

    # Pixelwise reliability weighting
    w_fg = p_fg * (1 - p_bg)
    w_bg = p_bg * (1 - p_fg)

    # Dempster–Shafer inspired adaptive fusion
    numerator = w_fg * (1 - p_bg)
    denominator = numerator + w_bg * (1 - p_fg) + eps
...
class UniSAM_predictor(nn.Module):
    def __init__(
        ...
        self.conv1fg = nn.Conv2d(in_channels=256, out_channels=1, kernel_size=1)
        self.conv1bg = nn.Conv2d(in_channels=256, out_channels=1, kernel_size=1)
    
    def set_support_images(self,test_image_list,test_mask_list,invert_masks=False):
     if invert_masks:
    ...
    def set_support_images_bg(self,test_image_list,test_mask_list,invert_masks=True):
     if invert_masks:
        test_mask_list = [1 - m for m in test_mask_list]
    ...
    # Assign weights from trained logistic regression
     self.conv1bg.weight.data = torch.from_numpy(self.model_LG.coef_.reshape(1, 256, 1, 1)).float()
     self.conv1bg.bias.data = torch.from_numpy(self.model_LG.intercept_).float()
     self.conv1bg = self.conv1bg.to(device)
    def set_support_images_fg(self,test_image_list,test_mask_list,invert_masks=False):
     if invert_masks:
        test_mask_list = [1 - m for m in test_mask_list]
    ...
    # Assign weights from trained logistic regression
     self.conv1fg.weight.data = torch.from_numpy(self.model_LG.coef_.reshape(1, 256, 1, 1)).float()
     self.conv1fg.bias.data = torch.from_numpy(self.model_LG.intercept_).float()
     self.conv1fg = self.conv1fg.to(device)
    ...
    # === Step 2: Run 1x1 convolution and apply sigmoid ===
     confidence_map_fg = torch.sigmoid(self.conv1fg(self.test_feat))  # [1, 1, H_feat, W_feat]
     confidence_map_bg = torch.sigmoid(self.conv1bg(self.test_feat))  # [1, 1, H_feat, W_feat]
     # === Step 3: Bayesian Fusion ===
     confidence_map = adaptive_dempster_fusion(confidence_map_fg, confidence_map_bg)
    ...
    def Image2ConfidenceMap_initial(self, test_image, predictor, model_LG, image_name='sample', verbose=True):
     '''
    Compute, visualize, and save semantic confidence maps and inputs.
    ...
    def predict_detailed(self,
        ...
        semantic_confidence_map = self.Image2ConfidenceMap_initial(test_image,self.predictor,self.model_LG,verbose = False)
